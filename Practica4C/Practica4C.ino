...
void blinkTask(void *param) {
    bool hornoIniciado = false;

    while(1){
     if (activarHorno && !hornoIniciado) {
            iniciarPerfil();       // ← esta función proviene del módulo .ino de control del horno
            hornoIniciado = true;
            Serial.println("Perfil de horneado iniciado.");
        }
    ...
    if (activarHorno) {
            actualizarPID(temperatura);
            actualizarZona();
        } else {
            digitalWrite(SSR_PIN, LOW);  // Seguridad: si no está activado, mantener apagado
        }
